import { 
  Github, Package, Container, Twitter, Instagram, Facebook, 
  Linkedin, Music, Youtube, Chrome, Code, 
...
const PLATFORM_ICON_MAP: Record<PlatformType, React.ComponentType<any>> = {
  ...
  // Stores
  'chrome-store': Chrome,
  'firefox-addons': Chrome, // Utiliser Chrome comme fallback
...
export const PlatformIcon = ({ platform, className = '', size = 'md' }: PlatformIconProps) => {
...
export const PlatformIconWithBackground = ({ 
  ...
  if (!IconComponent) {
    return (
      <div className={`${containerSizes[size]} rounded-lg bg-gray-400 flex items-center justify-center ${className}`}>
  ...
  return (
    <div className={`${containerSizes[size]} rounded-lg ${bgClass} flex items-center justify-center ${className}`}>
